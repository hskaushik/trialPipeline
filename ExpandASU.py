...
def ExpandASUToP1(pdbfname, na, nb, nc, incr):
  ...
  #the following 2 lines will output the filename in the original order which includes scaling factor.
  #outfname = outfname.split(".cif")[0] \
  # + "[" + str(na) + "," + str(nb) + "," + str(nc) + "]_infl" + str(Roundoff(incr,6)) +".pdb"
  #the following line is the edited version of the previous 2 lines to write only the pdb filename with "P1" suffix
  outfname = outfname.split(".cif")[0] +"_p1.pdb"
